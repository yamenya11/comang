    ...
    public class AngularASTBuilder extends AngularParserBaseVisitor<Node> {
        ...
        public AngularASTBuilder(SymbolTable symbolTable, ClassSymbolTable classSymbolTable, ErrorHandler errorHandler,Import importSymbols) {
        ...
        public Node visitIMPORTLABEL(AngularParser.IMPORTLABELContext ctx) {
            ...
            if (symbolTable.isImported(source)) {
                errorHandler.reportSemanticError("Duplicate import detected: " + source, ctx.start);
                return null;
            }
            symbolTable.addImported(source);

            SymbolEntry newEntry = new SymbolEntry(imported, SymbolType.IMPORT, source, scope);
            symbolTable.addSymbol(new SymbolEntry(imported, SymbolType.IMPORT, source, scope));
            symbolTable.addSymbol(newEntry);
            importSymbols.addSymbol(newEntry);
